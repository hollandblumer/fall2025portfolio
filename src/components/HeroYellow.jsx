...
const DRAG_ENABLED = false; // ⬅️ keep seam dragging disabled
  ...
  useEffect(() => {
    ...
    class SeamVeil extends THREE.Mesh {
      ...
      static makeVeilTexture(w, h) {
        ...
        // No brown — bright butter/yolk golds only
        g.addColorStop(0.0, "rgba(255,234,132,0.20)"); // #FFEA84
        g.addColorStop(0.5, "rgba(255,210,74,0.36)"); // #FFD24A
        g.addColorStop(1.0, "rgba(255,234,132,0.20)");
        ctx.fillStyle = g;
    ...
    async function bootP5() {
      // We run at ~30 fps; 3 seconds ≈ 90 frames
      const PAINT_FPS = 30;
      const EDGE_PHASE_FRAMES = Math.round(3 * PAINT_FPS); // ← block center ~3s
      const PAINT_FRAMES = EDGE_PHASE_FRAMES + 230; // keep painting a bit after
      ...
      // Bright yellows (no brown)
      const Y_LIGHT = "#FFEA84";
      const Y_DEEP = "#FFD24A";
      ...
      const vert = `
        ...
        void main(){ var.vertTexCoord = aTexCoord; gl_Position = uProjectionMatrix*uModelViewMatrix*vec4(aPosition,1.0); }
      `.replace("var.vertTexCoord", "var_vertTexCoord"); // small dev-friendly fix
      ...
      const sketch = (p) => {
        ...
        p.setup = () => {
          ...
          window.__p5UpdateHook = () => {
            ...
            if (paintCount < PAINT_FRAMES) {
              ...
              // occasional unifying wash — still yellow, no brown
        ...
        // ---- Edge-first bias: 3s (CENTER BLOCKED), then open smoothly ----
        const randX = () => {
          // During the first ~3s, spawn mostly in left/right bands
          if (paintCount < EDGE_PHASE_FRAMES * 0.6) {
            const leftBandMax = mySize * 0.35;
            const rightBandMin = mySize * 0.65;
            return p.random() < 0.5
              ? p.random(-mySize * 0.1, leftBandMax)
              : p.random(rightBandMin, mySize * 1.1);
          }
          return p.random(-mySize * 0.1, mySize * 1.1);
        };
        const randY = () => p.random(-mySize * 0.1, mySize * 1.1);

        // Block center for EDGE_PHASE_FRAMES, shrink blocked band to 0
        function centerIsBlocked(g, x) {
          if (paintCount >= EDGE_PHASE_FRAMES) return false;
          const t = p.constrain(paintCount / EDGE_PHASE_FRAMES, 0, 1);
          const ease = 1.0 - Math.pow(1.0 - t, 3.0); // ease-out
          const halfWidthAtStart = g.width * 0.22; // ~45% blocked center
          const halfWidthNow = halfWidthAtStart * (1.0 - ease);
          const cx = g.width * 0.5;
          return Math.abs(x - cx) < halfWidthNow;
        }
        ...
        function Particle(p, x, y) {
          ...
          this.show = (g) => {
            // Gate out the center until it opens
            if (centerIsBlocked(g, this.pos.x)) {
              // Nudge outward a bit; if still in gate, skip draw this frame
              const cx = g.width * 0.5;
              const dir = this.pos.x < cx ? -1 : 1;
              this.pos.x += dir * 6;
              if (centerIsBlocked(g, this.pos.x)) return;
            }
    ...
    async function bootThree() {
      ...
      renderer.setClearColor("#FFE37F"); // softer yellow background (no brown)
      ...
      // Drag handlers (wired only if DRAG_ENABLED)
    ...
    const startAboutFall = () => {
      ...
      if (el) {
        ...
        aboutFall.y = Math.round(rect.top + rect.height / 2);
      } else {
  ...
  return (
    <>
      <style>{`
        ...
        /* ---- TRUE FULL BLEED (no side padding/gutters) ---- */
        .hero-fullbleed{
          width:100vw;
          max-width:100vw;
        ...
        /* Ensure body default margin doesn't introduce gutters in SPA demos */
        body{ margin:0; }
      `}</style>
      ...
      >
        <style>{`
          .hero-canvas{ position:absolute; inset:0; z-index:0; } /* fills section exactly */
          ...
          .navtext button { position:absolute; inset:0; border:0; background:transparent; padding:0; cursor:pointer; }
          ...
          .restart-pad{ position:fixed; left:12px; bottom:12px; z-index:20; }
          .restart-pad button{
...
const styles = {
  hero: {
    ...
    maxWidth: "100vw",
    height: "100svh",
    background: "#FFE37F", // soft yellow background behind the wave
