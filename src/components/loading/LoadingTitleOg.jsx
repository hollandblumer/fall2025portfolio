    ...
    const maybeInit = () => {
      ...
      const sketch = (p) => {
        ...
        p.setup = () => {
          ...
          p.canvas.style.background = background;
          p.pixelDensity(1);
          p.noStroke();
          p.frameRate(60);
        ...
        function fitTextSizeToWidth(words, maxW, ff) {
          ...
          meas.textFont(ff);
          if (typeof meas.textStyle === "function") meas.textStyle(p.NORMAL);

          let baseGuess = contentH * 0.26;
          meas.textSize(baseGuess);
          ...
          let ts = baseGuess;
          if (widest > 0) ts = Math.min(baseGuess, (baseGuess * maxW) / widest);
          return Math.min(ts, contentH * 0.5);
        }
        ...
        function drawWord(pg, word, ts) {
          ...
          if (typeof pg.textStyle === "function") pg.textStyle(p.NORMAL);
          pg.textSize(ts);
        ...
        p.draw = () => {
          ...
          const baseW = p.lerp(LOAD_START_W, LOAD_END_W, u);

          let maxW;
          if (elapsed < morphEnd) {
            maxW = p.lerp(baseW, END_MAX_W, prog);
          } else {
            ...
            if (post <= POST_HOLD_MS) {
              ...
              maxW = p.lerp(END_MAX_W, BASE_MAX_W, r);
            } else {
              maxW = BASE_MAX_W;
              p.noLoop();
              onFinish && onFinish();
