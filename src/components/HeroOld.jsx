...
import heroImage from "../assets/images/h2-tile.png";
...
export default function Hero() {
  ...
  const p5HostRef = useRef(null); // unused now, but harmless
  ...
  useEffect(() => {
    ...
    class Wave extends THREE.Mesh {
      constructor(xCurve, zCurve, gridTex) {
        ...
        const mat = new THREE.ShaderMaterial({
          uniforms: {
            map: { value: gridTex }, // 🔸 our image texture
            offsetRepeat: {
              value: new THREE.Vector4(0.0, 0.0, 1.0, 1.6),
            },
            xCurve: { value: xCurve },
            zCurve: { value: zCurve },
          ...
          `,
          // 🔸 Fragment: just sample the image texture (our "image instead of shader")
          fragmentShader: `
        ...
        super(geom, mat);
        this.scrollSpeed = 0.05; // image scroll speed
      }
      update(dt) {
    ...
    async function bootThree() {
      ...
      // 🔸 Load image texture instead of using a p5 canvas
      const loader = new THREE.TextureLoader();
      tex = await new Promise((resolve, reject) => {
        loader.load(heroImage, resolve, undefined, reject);
      });
      ...
      const animate = () => {
        ...
        // flat → then ease fold + start scroll
        if (sinceBoot <= FIRST_HOLD_MS) {
          ...
          wave.scrollSpeed = lerp(0.0, TARGET_SCROLL, e); // start image scroll
        ...
        wave.update(dt); // moves offsetRepeat.y
        seamVeil.position.x = seamX();
    ...
    const restartAll = () => {
      // 🔸 restart intro timing so fold + scroll replays
  ...
  return (
    <>
      ...
      >
        {/* p5 host not used anymore, safe to delete later */}
        <div
          ref={p5HostRef}
          style={{
            position: "fixed",
